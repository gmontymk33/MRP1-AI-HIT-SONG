...
def get_all_midi_files2():
    # Ryan:
    # Slightly adjusted get_all_midi_files() method. Before, if no note was being played, it placed a copy of the previous
    # note but with note.type = "rest". Now, it treats a rest more like a note; it adds a note with note.note = -1 and
    # note.type = "note_on" when the rest starts and with note.type = "note_off" when it ends. I am not  sure if the use of
    # -1 as a note value can cause trouble in other methods, hence I also left the original method. Also only checked if it
    # worked for melody.
    midi_files = []
